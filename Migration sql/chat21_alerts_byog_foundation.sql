...
-- These are examples to run MANUALLY after applying. They are kept
-- commented so the migration itself never executes them (query 6 hits
-- cron.job, which does not exist unless pg_cron is enabled).
