...
-- ── 8. pg_cron — monthly quota reset ─────────────────────────────────────────
-- Runs at 00:00 IST on the 1st of every month (IST = UTC+5:30 → 18:30 UTC prev day).
-- Resets wa_messages_sent_month to 0 and updates wa_quota_reset_date.
-- Requires pg_cron extension — enable in Supabase Dashboard →
--   Database → Extensions → pg_cron → Enable.

SELECT cron.schedule(
  'reset-wa-monthly-quota',               -- job name (idempotent)
  '30 18 28-31 * *',                      -- 18:30 UTC = 00:00 IST; only runs on days
                                          -- 28-31, guarded by the WHERE below
  $$
    ...
    WHERE
      -- Only reset if we're actually in the last day of the month
      (NOW() + INTERVAL '1 day')::DATE = DATE_TRUNC('month', NOW() + INTERVAL '1 day')::DATE
      AND is_active = true;
  $$
);
...
-- Check new columns on students:
-- SELECT column_name, data_type, column_default
-- FROM information_schema.columns
-- WHERE table_name = 'students'
--   AND column_name = 'parent_phone_opted_out';

-- Check new columns on schools:
-- SELECT column_name, data_type, column_default
-- FROM information_schema.columns
-- WHERE table_name = 'schools'
--   AND column_name IN ('wa_monthly_quota','wa_messages_sent_month','wa_quota_reset_date');

-- Check wa_message_log table + constraints:
...
-- Check pg_cron job registered:
-- SELECT jobname, schedule, command FROM cron.job WHERE jobname = 'reset-wa-monthly-quota';

-- ============================================================
-- END OF MIGRATION
-- Next: supabase/functions/notify-attendance/index.ts (Session B)
-- ============================================================
