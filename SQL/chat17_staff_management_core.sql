-- ============================================================
-- SCHOOLIUM - CHAT 17 MIGRATION
-- Staff Management Module - CORE FOUNDATION
-- ============================================================
-- Contents:
--   1. profiles  - extend role CHECK (+principal, receptionist, staff),
--                  add last_login_at
--   2. schools   - staff_id_seq (employee number counter),
--                  staff_late_after_time (QR late cutoff)
--   3. tables    - staff, subjects, class_teachers, subject_assignments,
--                  staff_attendance, leave_requests, staff_documents,
--                  login_history, role_permissions
--   4. indexes
--   5. triggers  - updated_at, staff->profile sync, same-school guards
--   6. helpers   - get_my_staff_id, get_my_role, has_permission
--   7. RLS       - enable + policies on all new tables
--   8. storage   - private staff-docs bucket + policies
--   9. RPCs      - create_staff_member (service_role), set_staff_status,
--                  get_staff_directory_basic, mark_staff_attendance_bulk,
--                  record_staff_scan, apply_leave, cancel_leave,
--                  review_leave_request, get_teacher_assignments,
--                  get_staff_attendance_summary, get_staff_dashboard_stats,
--                  touch_login
--  10. seeds     - role_permissions global defaults
--  11. pg_cron   - purge login_history older than 180 days
--  12. NOTIFY pgrst reload
--
-- Rules honoured: pure ASCII, CREATE OR REPLACE, SECURITY DEFINER with
-- SET search_path + role/school verification + REVOKE/GRANT, RLS on
-- profiles NOT modified (columns added only), school_id isolation on
-- every table.
-- ============================================================
...
-- ============================================================
-- SECTION 12: RELOAD POSTGREST SCHEMA CACHE
-- ============================================================

NOTIFY pgrst, 'reload schema';

-- ============================================================
-- END OF CHAT 17 MIGRATION
-- ============================================================
