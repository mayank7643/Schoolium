...
BEGIN
  ...
  LOOP
    ...
    CASE v_item.frequency
      ...
      WHEN 'quarterly' THEN
        v_months := ARRAY[]::TEXT[];
        FOR v_m IN 1..ARRAY_LENGTH(COALESCE(v_item.quarterly_months, ARRAY[4,7,10,1]), 1) LOOP
          DECLARE
            v_qm INTEGER := COALESCE(v_item.quarterly_months, ARRAY[4,7,10,1])[v_m];
            v_qy INTEGER;
          BEGIN
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'auto-generate-monthly-dues') THEN
