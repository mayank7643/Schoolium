...
export default function FeeSummaryPage() {
  ...
  async function printPdf() {
    ...
    try {
      ...
      if (!blob || !frame) { setPrinting(false); return }
      const url = URL.createObjectURL(blob)
      // Print through the persistent hidden iframe (dynamically-created frames
      // make Chrome print the parent page instead).
      frame.onload = () => {
        try {
          frame.contentWindow?.focus()
          frame.contentWindow?.print()
        } catch { setError('Could not open print dialog') }
        setTimeout(() => URL.revokeObjectURL(url), 15000)
      }
      frame.src = url
    } catch { setError('Print failed') }
    setPrinting(false)
  }
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={printPdf} disabled={printing || loading} className="btn-secondary flex items-center gap-2 text-sm">
            {printing ? <><Loader2 size={15} className="animate-spin" /> Preparing…</> : <><Printer size={15} /> Print</>}
          </button>
      ...
      {/* Persistent hidden frame for printing the PDF directly */}
