...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase
      .from('fee_payments')
      .select('id, amount_paid, paid_date, receipt_number, students(full_name, student_uid), fee_dues(label, source, description)')
      .eq('school_id', schoolId)
  ...
  const recentActivity: ActivityItem[] = (recentStructuredRes.data ?? []).map((p: any) => {
    ...
    const label  = isMan
      ? (due?.description ?? 'Manual fee')
      : (due?.label ?? 'Fee payment')
    return {
      id:     p.id,
