...
// 'YYYY-MM' -> 'July 2026'. Falls back to the due date's month so the template
// variable is never empty (Meta rejects empty parameters at send time).
function fmtMonth(month: string | null | undefined, fallbackDate: string): string {
  if (month) {
    const [y, m] = month.split('-').map(Number)
    if (y && m) return new Date(y, m - 1, 1).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })
  }
  return new Date(fallbackDate).toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })
}
...
async function processReminder(
  ...
  const params = [
    ...
    student.full_name,
    fmtMonth(due.month, due.due_date),
