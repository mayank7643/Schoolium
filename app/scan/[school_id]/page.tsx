...
// Requires login — guards are auto-redirected here after login.
// Security: RLS ensures guards only see their own school's students.
// QR encodes Supabase student UUID (unguessable 32-char string).
...
export default function ScanPage() {
  ...
  // ── Step 1: Auth check — verify logged in + correct school ──
  useEffect(() => {
    async function checkAuth() {
      ...
      // Guard trying to access another school's scan page → block
  ...
  // ── Step 2: Load cache (only after auth confirmed) ──────────
  const loadCache = useCallback(async () => {
    ...
    try {
      const supabase = createClient()
      // RLS ensures this only returns students from guard's school
  ...
  // ── Realtime: other guards at same school ───────────────────
  useEffect(() => {
    if (!schoolId) return
    const supabase = createClient()
  ...
  const overlaySubtext =
    ...
    scanResult?.type === 'already' ? 'Scanned earlier today' : 'QR not recognised'
        ...
        {loadingCache ? (
          ...
          <>
            ...
            <div className="mt-4 flex items-center gap-2 bg-slate-800 rounded-full px-4 py-2">
              ...
              <span className="text-slate-300 text-xs">{cacheCount} students · {gate}</span>
            </div>
          </>
        )}
