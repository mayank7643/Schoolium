...
import {
  ...
  LogIn, LogOut as LogOutIcon, ArrowRight
} from 'lucide-react'
...
export default function AttendancePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [entryRes, exitRes, totalRes, guardsRes] = await Promise.all([
      ...
      supabase.from('profiles')
        .select('id, full_name, gate, is_active')
        .eq('school_id', sid)
        .eq('role', 'guard')
    ...
    setGuards((guardsRes.data ?? []) as GuardSummary[])
    setLoading(false)
  }, [])
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* Today's stats — 3 cards: entry, exit, absent */}
      <div className="grid grid-cols-3 gap-4 mb-6">
