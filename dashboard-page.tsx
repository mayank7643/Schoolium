...
import { Users, IndianRupee, BookOpen, AlertCircle, TrendingUp, TrendingDown, Minus, ArrowRight, CalendarDays } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const [
    ...
    // This month's manual fees (fees table)
    supabase
      .from('fees')
    ...
    // Last month's manual fees (paid) — for comparison
    supabase
      .from('fees')
    ...
    // 5 most recent manual payments for the activity feed
    supabase
      .from('fees')
    ...
    // Overdue manual fees — students who owe money
    supabase
      .from('fees')
    ...
    // Structured fee stats via RPC (fee_dues + fee_payments tables)
    supabase.rpc('get_fee_dashboard_stats', { p_school_id: schoolId }),

    // 5 most recent structured fee payments for activity feed
    supabase
      .from('fee_payments')
      .select('id, amount_paid, paid_date, created_at, receipt_number, students(full_name, student_uid), fee_dues(label)')
      .eq('school_id', schoolId)
      .order('created_at', { ascending: false })
      .limit(5),
  ])
