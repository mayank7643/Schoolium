...
import {
  ...
  ShieldAlert, KeyRound, Clock, Filter,
} from 'lucide-react'
...
export default function FeeCollectPage() {
  ...
  async function handleCollect() {
    ...
    // Helper: record one payment against a due
    async function payDue(dueId: string, amount: number): Promise<PaymentResult | null> {
    ...
    // 1. Pay current month dues (in order, prorate remaining)
    for (const due of currentDues) {
      if (remaining <= 0) break
      const toPay = Math.min(remaining, due.balance)
      const result = await payDue(due.due_id, toPay)
    ...
    for (const due of arrearDues) {
      ...
      if (!result) { setSaving(false); return }
      lines.push({
        label:          due.label + ` (${fmtMonth(due.month)})`,
        fee_type:       due.fee_type,
        amount:         toPay,
        receipt_number: result.receipt_number,
        is_arrear:      true,
    ...
    // 3. Pay late fee — if any and remaining > 0
    // Late fee is embedded in the dues' total_due already, so it's already
    // covered in step 1/2. We log it as audit but don't create a separate payment.
    // If collector waived any late fee, log to audit trail.
    if (lateFeeWaived > 0) {
    ...
    // 4. Create and pay extra manual dues
    for (const extra of extraFees) {
      if (remaining <= 0 || extra.amount <= 0) continue
      const toPay = Math.min(remaining, extra.amount)
      const curMonth = currentMonthStr()
      const acYear   = currentAcademicYear()

      // Create the manual due first
      ...
      const result = await payDue(newDueId, toPay)
      if (!result) { setSaving(false); return }
      lines.push({
        label:          extra.label,
    ...
    // Audit: student_loaded event was already fired when student was selected.
    // Fire payment_collected event now.
    await supabase.rpc('log_fee_audit_event', {
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 last:border-0">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            {due.due_month ? fmtMonth(due.due_month) : ''}
