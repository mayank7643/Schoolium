...
function buildReceiptHTML(params: {
  ...
  const { school, student, lines, amountPaid, method, paidDate, grandBalance } = params
  const formattedDate = new Date(paidDate + 'T00:00:00')
    .toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })
  const totalBilled = lines.reduce((s, l) => s + l.amount, 0)

  // Group lines: current dues, arrears, extras
  ...
  function lineRow(l: ReceiptLine) {
    const discountNote = l.discount_amount && l.discount_amount > 0
      ? ` <span style="color:#16a34a;font-size:11px">(-₹${l.discount_amount.toLocaleString('en-IN')} disc)</span>` : ''
    const lateFeeNote  = l.late_fee_amount && l.late_fee_amount > 0
      ? ` <span style="color:#b45309;font-size:11px">(+₹${l.late_fee_amount.toLocaleString('en-IN')} late)</span>` : ''
    return `<tr>
      <td style="padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:13px;color:#1e293b">
        ${l.label}${discountNote}${lateFeeNote}
      </td>
      <td style="padding:6px 0;border-bottom:1px solid #f1f5f9;font-size:13px;text-align:right;font-weight:600;color:#1e293b">
  ...
  function section(title: string, rows: ReceiptLine[]) {
    ...
    return `
      <tr><td colspan="2" style="padding:10px 0 4px;font-size:10px;font-weight:700;color:#64748b;letter-spacing:1.5px;text-transform:uppercase">${title}</td></tr>
...
.bar{background:#2563eb;height:6px;border-radius:4px;margin-bottom:24px}
...
.mrow{display:flex;justify-content:space-between;font-size:12px;padding:3px 0}
.ml{color:#64748b}.mv{font-weight:600}
table{width:100%;border-collapse:collapse}
th{font-size:10px;color:#94a3b8;text-align:left;padding-bottom:6px;letter-spacing:.5px}
th:last-child{text-align:right}
.total-row{display:flex;justify-content:space-between;font-size:13px;padding:5px 0;border-top:1px solid #e2e8f0;margin-top:4px}
.box{background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:14px 18px;margin:16px 0;display:flex;justify-content:space-between;align-items:center}
.bal{font-size:11px;color:#b45309;background:#fffbeb;border:1px solid #fde68a;border-radius:6px;padding:6px 10px;margin-top:4px}
.sig{display:flex;justify-content:flex-end;margin-top:24px;padding-top:14px;border-top:1px solid #e2e8f0}
.sigline{border-top:1.5px solid #475569;width:140px;padding-top:5px;font-size:10px;color:#64748b;text-align:center}
...
<div class="meta">
  ...
  ${student.student_uid ? `<div class="mrow"><span class="ml">ID</span><span class="mv" style="font-family:monospace">${student.student_uid}</span></div>` : ''}
  ...
  <div class="mrow"><span class="ml">Date</span><span class="mv">${formattedDate}</span></div>
</div>
<table>
...
<div class="total-row"><span style="color:#64748b">Total billed</span><span style="font-weight:700">₹${totalBilled.toLocaleString('en-IN')}</span></div>
<div class="box">
  <span style="font-size:11px;color:#1d4ed8;font-weight:700;text-transform:uppercase;letter-spacing:1px">Paid Now</span>
  <span style="font-size:26px;font-weight:800;color:#1e3a8a">₹${amountPaid.toLocaleString('en-IN')}</span>
</div>
...
function SuccessScreen({
  ...
  grandBalance, onCollectAnother, onClose,
}: {
  school: SchoolInfo
  student: StudentResult
  lines: ReceiptLine[]
  ...
  function handlePrint() {
    ...
    const html = buildReceiptHTML({ school, student, lines, amountPaid, method, paidDate, grandBalance })
...
export default function FeeCollectPage() {
  ...
  const loadBilling = useCallback(async (s: StudentResult) => {
    ...
    // Compute initial "amount being paid" = grand balance
    const grand = Number(firstRow.grand_balance ?? 0)
    setAmountStr(String(grand))
  ...
  async function handleCollect() {
    ...
    // ── Step 1: Create all manual dues first (before bulk payment call) ──────
    // Extra fees need a due_id before they can be paid.
    const extraDueIds: { due_id: string; label: string; fee_type: string; amount: number }[] = []
    for (const extra of extraFees) {
      if (remaining <= 0 || extra.amount <= 0) continue
      const curMonth = currentMonthStr()
      const acYear   = currentAcademicYear()
      const { data: dueData, error: dueError } = await supabase.rpc('create_manual_due', {
        p_school_id:     profile!.school_id,
        p_student_id:    student!.id,
        p_fee_type:      extra.fee_type,
        p_label:         extra.label,
        p_amount:        extra.amount,
        p_due_date:      paidDate,
        p_month:         curMonth,
        p_academic_year: acYear,
        p_notes:         notes || null,
      })
      if (dueError) { setSaveError(dueError.message); setSaving(false); return }
      const newDueId = Array.isArray(dueData) ? dueData[0]?.due_id : dueData?.due_id
      if (!newDueId) { setSaveError('Failed to create manual due'); setSaving(false); return }
      extraDueIds.push({ due_id: newDueId, label: extra.label, fee_type: extra.fee_type, amount: Math.min(remaining, extra.amount) })
      remaining -= Math.min(remaining, extra.amount)
    }

    // ── Step 2: Build a single payments array for the bulk RPC ───────────────
    // All dues (current month + arrears + extra) go in one call → one receipt number.
    remaining = parsedAmountPaid  // reset for the allocation pass below

    type BulkItem = { due_id: string; amount: number; meta: Omit<ReceiptLine, 'payment_id' | 'receipt_number'> }
    ...
    for (const ex of extraDueIds) {
      bulkItems.push({
        due_id: ex.due_id,
        amount: ex.amount,
        meta:   { label: ex.label, fee_type: ex.fee_type, amount: ex.amount,
    ...
    // ── Step 3: Single bulk RPC call → one shared receipt number ─────────────
    const { data: bulkData, error: bulkError } = await supabase.rpc('record_bulk_fee_payment', {
      p_school_id:      profile!.school_id,
      p_student_id:     student!.id,
      p_payments:       bulkItems.map(i => ({ due_id: i.due_id, amount: i.amount })),
    ...
    const bulkRows: { payment_id: string; receipt_number: string; due_id: string }[] =
      Array.isArray(bulkData) ? bulkData : []

    // Map results back to receipt lines using due_id
    for (const item of bulkItems) {
      ...
      lines.push({
        ...
        receipt_number: row.receipt_number,  // same for all lines in this session
      })
    }
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <SuccessScreen
          ...
          onCollectAnother={() => { setSuccessData(null); setExtraFees([]) }}
