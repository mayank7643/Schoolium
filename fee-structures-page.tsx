...
interface StructureWithItems extends FeeStructure {
  ...
  async function handleGenerateDues() {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/generate-fee-dues`,
        {
          method:  'POST',
          headers: {
          ...
          body: JSON.stringify({ fee_structure_id: editingStructure.id }),
        }
      )

      const json = await res.json()
      if (!res.ok) {
        setError(json.error ?? 'Failed to generate dues')
      } else {
