...
// Sprint 3 — Rebuild
// fees table is gone. This page is now:
//   - Navigation hub (Collect Fee, Structures, Defaulters, Reversals, EOD, Generate Dues)
//   - Recent payments list from fee_payments (structured + manual combined)
//   - Pending reversal alert banner for admin
...
import {
  ...
  ClipboardCheck, TrendingUp, Clock, ChevronDown,
} from 'lucide-react'
...
export default function FeesPage() {
  ...
  // Inline reversal request — keyed by payment id
  const [reversalPaymentId, setReversalPaymentId] = useState<string | null>(null)
  const [reversalReason,    setReversalReason]    = useState('')
  const [reversalSaving,    setReversalSaving]    = useState(false)
  const [reversalError,     setReversalError]     = useState('')
  const [reversalDoneIds,   setReversalDoneIds]   = useState<Set<string>>(new Set())
  ...
  const fetchData = useCallback(async () => {
    ...
    const profileRes = await supabase
      .from('profiles')
      .select('school_id, role, schools(id)')
      .single()
    ...
    const [paymentsRes, reversalsRes, statsRes] = await Promise.all([
      // Recent 50 payments from fee_payments — single source of truth
      supabase
        .from('fee_payments')
      ...
      // Pending reversals — admin needs to action these
      admin ? supabase
    ...
    const statsData = (statsRes.data ?? [])[0] as StatsRow | undefined
  ...
  async function handleSubmitReversal(paymentId: string) {
    ...
    setReversalDoneIds(prev => new Set(prev).add(paymentId))
  ...
  // Filter out counter-transaction reversal rows from the display list
  const displayPayments = payments.filter(p => !p.receipt_number?.startsWith('REV-'))
  const reversalPayments = payments.filter(p => p.receipt_number?.startsWith('REV-'))
  ...
  return (
    <div className="max-w-5xl mx-auto">
              ...
              return (
                <div key={r.id} className="bg-white border border-amber-100 rounded-xl px-3 py-2.5 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {fp?.amount_paid && (
                      <span className="text-sm font-semibold text-red-600">{fmt(fp.amount_paid)}</span>
                    )}
                    <Link
                      href="/dashboard/fees/reversals"
      ...
      {/* Generate dues result/error banners */}
      {genResult && (
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-50">
            {displayPayments.map(p => {
              ...
              return (
                <div key={p.id} className={`flex items-center gap-3 px-5 py-3 ${isReversed ? 'opacity-50' : ''}`}>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                    isReversed ? 'bg-slate-100' : 'bg-brand-50'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {isManual && (
                        <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded">Manual</span>
                      )}
                ...
                {/* Reversal request — expanded inline */}
                {!isReversed && !isPending && (
                  reversalDoneIds.includes(p.id) ? (
                    <div className="mt-2 flex items-center gap-2 text-xs text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg">
                      <AlertCircle size={11} className="shrink-0" /> Reversal requested — awaiting admin approval
                    </div>
                    ...
                    <div className="mt-2 border border-red-100 bg-red-50 rounded-xl p-3 flex flex-col gap-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => { setReversalPaymentId(null); setReversalReason(''); setReversalError('') }}
