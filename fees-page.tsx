                ...
                <div key={g.receipt_number ?? g.lines[0]?.id} className="px-5 py-3">
                  <div className={`flex items-center gap-3 ${isReversed ? 'opacity-50' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Single line: show label. Multi-line: show count + breakdown */}
                      {multiLine ? (
                        <div className="mt-0.5">
                          <p className="text-xs text-slate-400">
                            {g.lines.length} items · {g.receipt_number ?? ''} · {g.paid_date ? fmtDate(g.paid_date) : ''}
                          </p>
                          <div className="mt-1 flex flex-col gap-0.5">
                            {g.lines.map(l => (
                              <p key={l.id} className="text-[11px] text-slate-400 truncate">
                                · {(l.fee_dues as any)?.label ?? 'Fee payment'} — {fmt(l.amount_paid)}
                              </p>
                            ))}
                          </div>
                        </div>
                        ...
                        <p className="text-xs text-slate-400 truncate">
                          {(g.lines[0]?.fee_dues as any)?.label ?? 'Fee payment'}
