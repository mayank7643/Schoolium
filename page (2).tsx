...
interface ManualFee {
  ...
  created_at: string
  notes: string | null
  // Derived from balance: 'paid' | 'partial' | 'unpaid'
  status: string
  // Most recent receipt from fee_payments (joined)
  last_receipt: string | null
  last_payment_date: string | null
}
    ...
    <div className={`flex items-center justify-between px-4 py-3 border-b border-slate-100 last:border-0 ${
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p className="font-medium text-slate-800 text-sm capitalize">{fee.description}</p>
          <span className={
            fee.status === 'paid'    ? 'badge-green' :
            fee.status === 'partial' ? 'badge-yellow' : 'badge-red'
          }>{fee.status}</span>
          {/* Manual tag */}
        ...
        <div className="flex items-center gap-2 mt-0.5 text-xs text-slate-400 flex-wrap">
          ...
          {fee.last_payment_date && <span>· Paid {formatDate(fee.last_payment_date)}</span>}
          {fee.last_receipt && (
      ...
      <div className="text-right shrink-0 ml-2">
        {fee.status === 'paid' ? (
          ...
          <>
            <p className="font-semibold text-slate-700">{formatCurrency(fee.balance)}</p>
            {fee.amount_paid > 0 && (
              <p className="text-[10px] text-slate-400">of {formatCurrency(fee.amount)}</p>
            )}
          </>
        )}
      </div>
...
export default function StudentFeeLedgerPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    // Derive status + last receipt for manual dues from fee_payments already fetched
    const rawManual = (manualRes.data ?? []) as any[]
    const manualPaymentsMap = ((paymentsRes.data ?? []) as LedgerPayment[]).reduce<
      Record<string, LedgerPayment[]>
    >((acc, p) => {
      if (!acc[p.fee_due_id]) acc[p.fee_due_id] = []
      acc[p.fee_due_id].push(p)
      return acc
    }, {})

    const mappedManual: ManualFee[] = rawManual.map(d => {
      const duePayments = manualPaymentsMap[d.id] ?? []
      const sortedPmts  = [...duePayments].sort((a, b) =>
        b.paid_date.localeCompare(a.paid_date))
      const balance     = Number(d.balance)
      const status      = balance === 0 ? 'paid'
                        : Number(d.amount_paid) > 0 ? 'partial'
                        : 'unpaid'
      return {
        id:                  d.id,
        description:         d.label ?? '—',
        ...
        notes:               d.notes ?? null,
        status,
        last_receipt:        sortedPmts[0]?.receipt_number ?? null,
        last_payment_date:   sortedPmts[0]?.paid_date ?? null,
      }
    })

    setManualFees(mappedManual)
  ...
  // Manual totals (balance is GENERATED ALWAYS from fee_dues)
  const manualTotal   = manualFees.reduce((s, f) => s + f.amount,       0)
  const manualPaid    = manualFees.reduce((s, f) => s + f.amount_paid,   0)
  const manualPending = manualFees.reduce((s, f) => s + f.balance,       0)
  ...
  const timelineEntries: TimelineEntry[] = [
    ...
    ...manualFees.map(f => ({
      key:      `manual-${f.id}`,
      sortDate: f.last_payment_date ?? f.due_date ?? f.created_at.split('T')[0],
            ...
            <div className="flex flex-col gap-4">
              {Object.entries(timelineByMonth).map(([monthLabel, entries]) => {
                ...
                const monthPaid = entries.reduce((s, e) => {
                  ...
                  if (e.source === 'manual' && e.manual && e.manual.status === 'paid') return s + e.manual.amount_paid
                  return s
                }, 0)
                const monthBalance = entries.reduce((s, e) => {
                  if (e.source === 'structured' && e.due) return s + Number(e.due.balance)
                  if (e.source === 'manual' && e.manual && e.manual.status !== 'paid') return s + e.manual.balance
                  return s
                }, 0)
