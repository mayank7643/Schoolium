...
export default function ScanPage() {
  ...
  const syncQueue = useCallback(async () => {
    ...
    try {
      ...
      const rows = items.map(({ item }) => ({
        ...
        entry_type: item.entry_type ?? 'entry',   // default 'entry' for old queued items
      }))

      // Upsert attendance rows — ignoreDuplicates means a second scan for the
      // same student+day+entry_type is silently dropped (no error, no WA alert)
      const { data: upserted, error } = await supabase
        .from('attendance')
        .upsert(rows, {
          onConflict: 'school_id,student_id,scan_date,entry_type',
          ignoreDuplicates: true,
        })
        .select('id, student_id, school_id, scan_time, entry_type, gate')

      if (!error) {
        await removeFromQueue(items.map(i => i.key))
        setQueueSize(0)

        // Fire WhatsApp alert for each row that was actually inserted.
        // ignoreDuplicates: true means upserted only contains NEW rows —
        // duplicate scans return empty array, so no duplicate WA messages.
        // Fire-and-forget: never await — WA failure must not block next scan.
        if (upserted && upserted.length > 0) {
          fireWhatsAppAlerts(upserted)
        }
      }
    } catch (_) {}
  }, [])
  ...
  // Called after attendance rows are confirmed saved in Supabase.
  // Each call is independent — one failure does not affect others.
  // Uses the guard's current session token to authenticate the Edge Function.
  function fireWhatsAppAlerts(rows: Array<{
    id: string
    student_id: string
    school_id: string
    scan_time: string
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!session?.access_token) return  // no token = skip silently
      rows.forEach(row => {
        fetch(
          `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/notify-attendance`,
          {
            method:  'POST',
            headers: {
          ...
          // Network error calling Edge Function — scan is already saved,
          // retry job will pick up the failed wa_message_log row later.
        })
      })
    })
  }
